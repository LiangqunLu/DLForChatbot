...
####part I clean data and split (English to Chinese)
##input is English and Output is Chinese
...
def clean_text_complete():
    ...
    # load dataset
    raw_dataset = load_clean_sentences('model/english-chinese.pkl') #19777
    ...
    train, test = dataset[:int(n_sentences * 0.9)], dataset[int(n_sentences * 0.9):]
    # save
...
def language_model_training():
    ...
    # load datasets
    dataset = load_clean_sentences('model/english-chinese-both.pkl')
    test = load_clean_sentences('model/english-chinese-train.pkl')
    train = load_clean_sentences('model/english-chinese-test.pkl')
    ...
    # fit model
    filename = 'E_C_translation_model.h5'
    ...
    model.save('./model/E_C_translation_model.h5')
...
def txt_translation():
    ...
    # load datasets
    dataset = load_clean_sentences(sys.path[-1] + './model/english-chinese-both.pkl')
    train = load_clean_sentences(sys.path[-1] + './model/english-chinese-train.pkl')
    test = load_clean_sentences(sys.path[-1] + './model/english-chinese-test.pkl')
    # prepare english tokenizer
    eng_tokenizer = create_tokenizer(dataset[:, 0])
    eng_vocab_size = len(eng_tokenizer.word_index) + 1
    eng_length = max_length(dataset[:, 0])
    ...
    # load model
    model = load_model(sys.path[-1] + './model/E_C_translation_model.h5')
